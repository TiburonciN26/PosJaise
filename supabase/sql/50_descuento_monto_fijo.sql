-- =========================================================
-- POS Negocio 2 — Fase 11: descuento en Ventas por monto fijo
-- (además del ya existente por porcentaje)
-- Ejecutar en Supabase → SQL Editor → New query
-- Requiere que 49_atenciones_en_ventas_y_descuento.sql ya se haya
-- corrido (confirmar_venta con 5 parámetros, ventas.descuento_pct).
--
-- El botón de descuento en Ventas ahora alterna entre % y S/ — el
-- servidor recibe cuál de los dos se usó y calcula el total con el
-- que corresponda, nunca ambos a la vez (mutuamente excluyentes,
-- validado acá para no confiar en que el navegador mande solo uno).
-- =========================================================

begin;

alter table public.ventas
  add column descuento_monto numeric(10, 2) not null default 0
    check (descuento_monto >= 0);

drop function if exists public.confirmar_venta(text, numeric, jsonb, uuid, numeric);
...
begin
  ...
  insert into public.ventas
    (codigo, total, metodo_pago, monto_recibido, vendedor_id, cliente_id, descuento_pct, descuento_monto)
  values
    (v_codigo, v_total, p_metodo_pago, p_monto_recibido, auth.uid(), p_cliente_id, p_descuento_pct, p_descuento_monto)
...
grant execute on function public.confirmar_venta(text, numeric, jsonb, uuid, numeric, numeric) to authenticated;
