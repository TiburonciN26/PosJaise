-- =========================================================
-- POS Negocio 2 — Fase 12: monto a digitar en POS para ventas con tarjeta
-- Ejecutar en Supabase → SQL Editor → New query
-- Requiere que 50_descuento_monto_fijo.sql ya se haya corrido
-- (confirmar_venta con 6 parámetros, ventas.descuento_monto).
--
-- Culqi (POS físico) cobra una comisión (% + IGV, o un mínimo fijo + IGV
-- si el % no la alcanza) sobre el monto que se digita en el terminal.
-- Para que el negocio reciba el total de la venta intacto, el front
-- calcula cuánto hay que digitar en el POS (monto_pos_tarjeta) y ese
-- valor se guarda aparte del total real — el total sigue siendo el
-- valor de la venta, sin inflar reportes ni métricas de ingresos.
-- =========================================================

begin;

alter table public.ventas
  add column if not exists monto_pos_tarjeta numeric(10, 2) null;

alter table public.ventas
  drop constraint if exists ventas_monto_pos_tarjeta_check;

alter table public.ventas
  add constraint ventas_monto_pos_tarjeta_check
    check (monto_pos_tarjeta is null or monto_pos_tarjeta >= 0);

drop function if exists public.confirmar_venta(text, numeric, jsonb, uuid, numeric, numeric);
